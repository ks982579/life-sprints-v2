...
export function AnnualRecurring() {
  const { activities, loading, error, handleCreate, handleUpdate, handleDelete } = useRecurringItems(RecurrenceType.Annual);
  ...
  const handleSave = async (data: {
    ...
    if (editingActivity) {
      ...
      await handleCreate({ ...data, isRecurring: true, recurrenceType: RecurrenceType.Annual });
    }
    setShowEditor(false);
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <h2 className={styles.pageTitle}>Annual Recurring Items</h2>
      ...
      {showEditor && (
        <ActivityEditor
          ...
          fixedRecurrenceType={RecurrenceType.Annual}
        />
      )}
