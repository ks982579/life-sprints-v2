...
export function DailyRecurring() {
  const { activities, loading, error, handleCreate, handleUpdate, handleDelete } = useRecurringItems(RecurrenceType.Daily);
  ...
  const handleSave = async (data: {
    ...
    if (editingActivity) {
      ...
      await handleCreate({ ...data, isRecurring: true, recurrenceType: RecurrenceType.Daily });
    }
    setShowEditor(false);
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <h2 className={styles.pageTitle}>Daily Recurring Items</h2>
      ...
      {showEditor && (
        <ActivityEditor
          ...
          fixedRecurrenceType={RecurrenceType.Daily}
        />
      )}
