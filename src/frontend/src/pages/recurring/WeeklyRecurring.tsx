...
export function WeeklyRecurring() {
  const { activities, loading, error, handleCreate, handleUpdate, handleDelete } = useRecurringItems(RecurrenceType.Weekly);
  ...
  const handleSave = async (data: {
    ...
    if (editingActivity) {
      ...
      await handleCreate({ ...data, isRecurring: true, recurrenceType: RecurrenceType.Weekly });
    }
    setShowEditor(false);
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <h2 className={styles.pageTitle}>Weekly Recurring Items</h2>
      ...
      {showEditor && (
        <ActivityEditor
          ...
          fixedRecurrenceType={RecurrenceType.Weekly}
        />
      )}
